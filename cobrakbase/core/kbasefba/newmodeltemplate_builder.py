...
from cobrakbase.core.kbasefba.newmodeltemplate import NewModelTemplate, TemplateCompartment
...
class NewModelTemplateBuilder(MSTemplateBuilder):
    ...
    def __init__(self, template_id, name='', domain='', template_type='', version=1, info=None,
        ...
        super().__init__(template_id, name, domain, template_type, version, info,
                 biochemistry, biomasses, pathways, subsystems)
    ...
    def from_dict(d, info=None, args=None):
        ...
        builder = NewModelTemplateBuilder(d['id'], d['name'], d['domain'], d['type'], version, info)
    ...
    def build(self):
        ...
        template.add_compartments(list(map(lambda x: TemplateCompartment.from_dict(x), self.compartments)))

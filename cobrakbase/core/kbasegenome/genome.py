...
from cobrakbase.core.kbasegenome.genome_cds_feature import KBaseGenomeFeature, KBaseGenomeCDS
...
class KBaseGenome(KBaseObject, MSGenome):
    ...
    def __init__(self, genome_id: str, scientific_name: str, domain: str,
                 genome_tiers: list, feature_counts: dict, genetic_code: int,
                 dna_size: int, num_contigs: int, molecule_type: str,
                 source: str, md5: str, gc_content: float, taxonomy=None,
                 assembly_ref=None, taxon_ref=None,
                 info=None, args=None):
        """
...
@metadata ws suspect as Suspect Genome
  ...
  @optional list<string> contig_ids;
    ...
    def add_cdss(self, cds_list: list):
        ...
        if len(duplicates) > 0:
            raise ValueError(f"unable to add cdss {duplicates} already present in the genome")
    ...
    def from_kbase_data(kbase_data: dict, info=None, args=None):
        genome = KBaseGenome(kbase_data['id'],
                             kbase_data['scientific_name'], kbase_data['domain'], kbase_data['genome_tiers'],
                             kbase_data['feature_counts'], kbase_data['genetic_code'], kbase_data['dna_size'],
                             kbase_data['num_contigs'], kbase_data['molecule_type'], kbase_data['source'],
                             kbase_data['md5'], kbase_data['gc_content'],
                             kbase_data.get('taxonomy', None),
                             kbase_data.get('assembly_ref', None), kbase_data.get('taxon_ref', None),
                             info, args)
        features = [KBaseGenomeFeature.from_kbase_data(o) for o in kbase_data['features']]

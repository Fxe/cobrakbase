...
to a protein (minus introns and UTRs).
    ...
    def __init__(self, cds_id: str, protein_translation: str, dna_sequence: str, location: list):
        """
...
class KBaseGenomeFeature(MSFeature):
    ...
    def __init__(self, feature_id: str, protein_translation: str, dna_sequence: str,
                 location, cdss: list, functions):
        """
          Feature_id id;
  ...
  @optional list<tuple<string, string>> db_xrefs;
        ...
        MSFeature.__init__(self, feature_id, protein_translation, '; '.join(self.functions))
        self.location = location
    ...
    def _repr_html_(self):
        ...
        for term in self.ontology_terms:
            ots += "<tr><td><strong>{}</strong></td><td>{}</td></tr>".format(term, '; '.join(self.ontology_terms[term]))
        return """
        <table>
            <tr>

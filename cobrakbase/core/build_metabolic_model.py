...
def NewModelTemplateReactionToReaction(
    template_reaction, gpr, genome, template, index="0"
):
        ...
        cpd = ModelCompound(
            {
                ...
                "modelcompartment_ref": comp_compound.templatecompartment_ref
                + str(index),
...
def build_model_from_some_map(
    model_id, rxn_ids, genome, template, index="0", allow_all_non_grp_reactions=False
):
    model_base = {
        "id": model_id,
        "name": model_id,
    ...
    for rxn_id in rxn_ids:
        ...
        model_reaction = NewModelTemplateReactionToReaction(
            template_reaction, rxn_ids[rxn_id], genome, template, index
        )
    ...
    for rxn in template.reactions:
        if rxn.data["type"] == "universal" or rxn.data["type"] == "spontaneous":
            ...
            if (
                len(metabolites_in_model & res_metabolite_ids) > 0
                or allow_all_non_grp_reactions
            ) and res.id not in reactions_in_model:
...
def build_metabolic_model(
    model_id, genome, template, index="0", allow_all_non_grp_reactions=False
):
    rast = RastClient()
    ...
    (
        ...
        search_name_to_original,
    ) = modelseedpy.core.rast_client.aux_rast_result(res, g)
    ...
    cobra_model = build_model_from_some_map(
        model_id,
        metabolic_reactions_2,
        genome,
        template,

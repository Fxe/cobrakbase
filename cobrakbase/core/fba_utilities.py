...
class KBaseFBAUtilities():
    ...
    def add_drain_from_metabolite_id(self,cpd_id,lower_bound = None,upper_bound = None,prefix = 'EX_', prefix_name = 'Exchange for ', sbo = 'SBO:0000627'):
        bound_hash = self.media_const_hash()
        if lower_bound == None:
            lower_bound = -1*self.default_uptake
            if cpd_id in bound_hash:
                lower_bound = bound_hash[cpd_id]["lb"]
        if upper_bound == None:
            upper_bound = self.default_excretion
            if cpd_id in bound_hash:
                upper_bound = bound_hash[cpd_id]["ub"]                            
        if cpd_id in self.cobramodel.metabolites:
            id = prefix + cpd_id
            cobra_metabolite = self.cobramodel.metabolites.get_by_id(cpd_id)
    ...
    def apply_media_to_model(self,media = None,default_uptake = None,default_excretion = None):
        self.media = media
    ...
    def add_reversibility_binary_constraints(self,reaction_filter = None):
        ...
        for reaction in self.cobramodel.reactions:
            if reaction.id not in self.reversibility_binary and (reaction_filter == None or reaction.id in reaction_filter):
                self.reversibility_binary[reaction.id] = self.cobramodel.problem.Variable(reaction.id+"_rb", lb=0,ub=1,type="binary")
                ...
                self.reversibility_binary_constraints[reaction.id]["ff"] = self.cobramodel.problem.Constraint(
                    1000*self.reversibility_binary[reaction.id] - reaction.forward_variable,lb=0,ub=None,name=reaction.id+"_FB"
                )
                self.cobramodel.add_cons_vars(self.reversibility_binary_constraints[reaction.id]["ff"])
                self.reversibility_binary_constraints[reaction.id]["rf"] = self.cobramodel.problem.Constraint(
                    -1000*self.reversibility_binary[reaction.id] - reaction.reverse_variable,lb=-1000,ub=None,name=reaction.id+"_RB"
                )
                self.cobramodel.add_cons_vars(self.reversibility_binary_constraints[reaction.id]["rf"])
    ...
    def add_simple_thermo_constraints(self):
        ...
        for metabolite in self.cobramodel.metabolites:
            if metabolite.id not in self.potential_variables:
                self.potential_variables[metabolite.id] = self.cobramodel.problem.Variable(metabolite.id+"_u", lb=0,ub=1000)
                self.cobramodel.add_cons_vars(self.potential_variables[metabolite.id])
        #Adding thermodynamic constraints
        for reaction in self.cobramodel.reactions:
            if reaction.id not in self.simple_thermo_constraints and reaction.id[0:3].lower() != "ex_" and reaction.id[0:3].lower() != "dm_":
                if reaction.id not in self.reversibility_binary:
                    ...
                    self.cobramodel.add_cons_vars(self.reversibility_binary_constraints[reaction.id]["rf"])
                self.simple_thermo_constraints[reaction.id] = self.cobramodel.problem.Constraint(
                    Zero,lb=0,ub=1000,name=reaction.id+"_therm"
                )
                self.cobramodel.add_cons_vars(self.simple_thermo_constraints[reaction.id])
                self.cobramodel.solver.update()
                const_coef = {self.reversibility_binary[reaction.id] : 1000}
                for metabolite in reaction.metabolites:
                    const_coef[self.potential_variables[metabolite.id]] = reaction.metabolites[metabolite]
                self.simple_thermo_constraints[reaction.id].set_linear_coefficients(const_coef)
        #Updating solver one final time
        self.cobramodel.solver.update()
        
    def add_intracellular_metabolomics_constraints(self,peakstring,builder):
        drain_fluxes = list()
            ...
            if len(sub_array) > 2:
                ...
                if pfound == 1:
                    if peakid not in self.metabolomics_peak_variables:
                        ...
                        self.cobramodel.add_cons_vars(self.metabolomics_peak_variables[peakid])
                        self.metabolomics_peak_constraints[peakid] = self.cobramodel.problem.Constraint(
                            Zero,lb=0,ub=None,name=peakid
                        )
                        self.cobramodel.add_cons_vars(self.metabolomics_peak_constraints[peakid])
        ...
        for i in range(0,1000):
            if indexlist[i] > 10:
                if extend_with_template == 1:
                    new_penalties = self.temp_extend_model_index_for_gapfilling(i,input_templates)
    ...
    def mdl_extend_model_index_for_gapfilling(self,index,source_model,model_penalty):
        ...
        for modelcompound in source_model.metabolites:
            ...
            if cobra_metabolite.id not in self.cobramodel.metabolites and cobra_metabolite.id not in new_metabolites:
    ...
    def convert_modelreaction_stoichiometry(self, reaction):
        ...
        for metabolite_id in s:
            if metabolite_id in self.metabolites_remap:
                object_stoichiometry[self.cobramodel.metabolites.get_by_id(self.metabolites_remap[metabolite_id])] = s[metabolite_id]
        ...
        for reaction in self.cobramodel.reactions:
            if reaction.id in penalty_hash:
                ...
                if "reverse" in penalty_hash[reaction.id]:
                    obj_coef[reaction.reverse_variable] = abs(penalty_hash[reaction.id]["reverse"])
                elif default_penalty != 0:
                    obj_coef[reaction.reverse_variable] = default_penalty
                if "forward" in penalty_hash[reaction.id]:
                    obj_coef[reaction.forward_variable] = abs(penalty_hash[reaction.id]["forward"])
                elif default_penalty != 0:

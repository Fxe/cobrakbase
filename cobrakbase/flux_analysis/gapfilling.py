...
def gapfill(model, template=None, media=None, kbmodel=None, kbase_api=None):
...
def insert_gf(kbmodel, output_2, template_json):
    ...
    for rxn_id in output_2["reversed"]:
        list(filter(lambda x: x["id"] == rxn_id, model_json_gf["modelreactions"]))[0][
            "direction"
        ] = output_2["reversed"][rxn_id]
    for rxn_id in output_2["new"]:
        rxn = list(
            filter(lambda x: x["id"] == rxn_id[:-1], template_json["reactions"])
        )[0]
        model_reaction_reagents = []
        for trr in rxn["templateReactionReagents"]:
            cc = list(
                filter(
                    lambda x: x["id"] == trr["templatecompcompound_ref"].split("/")[-1],
                    template_json["compcompounds"],
                )
            )[0]
            modelcompounds = list(
                filter(
                    lambda x: x["id"] == cc["id"] + "0", model_json_gf["modelcompounds"]
                )
            )
            if len(modelcompounds) == 0:
                tc = list(
                    filter(
                        lambda x: x["id"] == cc["templatecompound_ref"].split("/")[-1],
                        template_json["compounds"],
                    )
                )[0]
                ...
                modelcompounds = list(
                    filter(
                        lambda x: x["id"] == cc["id"] + "0",
                        model_json_gf["modelcompounds"],
                    )
                )
            if len(modelcompounds) == 1:
                mrr = {
                    "coefficient": trr["coefficient"],
                    "modelcompound_ref": "~/modelcompounds/id/"
                    + modelcompounds[0]["id"],
                }
    ...
    model_gf = (
        FBAModelBuilder(model_json_gf, oi)

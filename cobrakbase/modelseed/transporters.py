...
def is_transport_abc(rxn):
    ...
    for o in parts:
        compounds = set(parts[o])
        #print(compounds)
        if 'cpd00002' in compounds and 'cpd00008' in compounds and 'cpd00009' in compounds:
            if inside == None:
                inside = o
            else:
                print('multiple match')
    ...
    if transport_compound == None or not transport_compound in parts[outside] or not transport_compound in parts[inside]:
        return (False, None, None, None, None)
...
class TransportersPipeline():
    ...
    def f1(self, df_abc, t_abc, ban, is_transport_function):
        compounds_to_reaction = self.get_compounds_to_reaction(t_abc, ban, is_transport_function)
            ...
            if not pd.isna(d['compound']):
                for cpd_id in d['compound'].split('/'):
                    ...
                    if cpd_id in compounds_to_reaction:
                        ...
                        role_reactions[d['roles']][cpd_id] = compounds_to_reaction[cpd_id]
                    else:
